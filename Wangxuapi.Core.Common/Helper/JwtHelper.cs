    ...
    public class JwtHelper
    {
        ...
        public static string IssueJwt(TokenModel tokenModel)
        {
            ...
            var claims = new List<Claim>
                 ...
                 */
                new Claim(JwtRegisteredClaimNames.Jti,tokenModel.UserId.ToString()),//JWT ID用于标识该JWT
                new Claim(JwtRegisteredClaimNames.Aud,tokenModel.Name.ToString()),//用户
                new Claim(JwtRegisteredClaimNames.Iat, $"{new DateTimeOffset(DateTime.Now).ToUnixTimeSeconds()}"),//发布时间
                new Claim(JwtRegisteredClaimNames.Nbf,$"{new DateTimeOffset(DateTime.Now).ToUnixTimeSeconds()}") ,//在此之前不可用
                //这个就是过期时间，目前是过期1000秒，可自定义，注意JWT有自己的缓冲过期时间
                new Claim (JwtRegisteredClaimNames.Exp,$"{new DateTimeOffset(DateTime.Now.AddSeconds(1000)).ToUnixTimeSeconds()}"),//到期时间
            ...
            // 可以将一个用户的多个角色全部赋予；
            claims.AddRange(tokenModel.Name.Split(',').Select(s => new Claim(ClaimTypes.Role, s)));
